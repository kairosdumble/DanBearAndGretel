...
const geminiService = require("../services/gemini_image.service");
...
async function extractTaxiMeterImage(req, res) {
  try {
    if (!req.file) {
    ...
    // 서비스 호출
    const finalFare = await geminiService.extractTaxiFare(buffer, mimetype);
